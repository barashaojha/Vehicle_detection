...
cap=cv2.VideoCapture('inshot/video (1).mp4')

frame_width = int(cap.get(3))
frame_height = int(cap.get(4))

out = cv2.VideoWriter('output.mp4', cv2.VideoWriter_fourcc('N', 'J', 'P', 'G'), 10, (frame_width, frame_height))

model=YOLO("Datasets/runs/detect/train/weights/best.pt")
classNames = ["tractor", "car", "bike", "rickshaw", "bus", "traveller", "truck", "Auto-rickshaw", "tempo"]
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            print(x1, y1, x2, y2)
            # Syntax to draw a rectangle on any image using opencv: cv2.rectangle(image, start_point, end_point, color, thickness)
